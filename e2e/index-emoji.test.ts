import { EVEN_PREFIX, ODD_PREFIX } from "../src/controller";
import { initializeServer } from "../src/server";
import { Server } from "@hapi/hapi";

describe("E2E Emoji Tests", () => {
  let server: Server;
  type Item = {
    id: number;
    name: string;
  ...
  describe("Basic Items functionality", () => {
      ...
      expect(response2.result).toEqual([
        {
          ...
          name: expect.stringContaining("Item 1"),
          price: 10,
        },
      ]);
    });
    ...
    it("should be able to update an item but not change the emoji", async () => {
      /* If the name property start with another emoji it is not replaced,
         the API insert the correct emoji as a prefix based on the name.
       */
      ...
      const response = await server.inject({
        ...
        payload: {
          name: `${theOtherPrefix} Item 1 updated`,
          price: 20,
        },
      });
      ...
      expect(response.result).toEqual({
        id: createdItem!.id,
        name: expect.stringContaining(`${itemPrefix} Item 1 updated`),
        price: 20,
      });
